function init_deparment_and_courses(
  ...
  fetch("http://coursescheduler.site/api/departments")
...
function load_courses(select_list, department) {
  ...
  courses = [];
  json = {};
  fetch("http://coursescheduler.site/api/" + department)
    .then((response) => response.json())
    .then((data) => {
...
async function getSections(department, course_code) {
  let response = await fetch(
    "http://coursescheduler.site/api/" + department + "/" + course_code
  );
...
async function fetchAvaliablePlans(jsonString) {
  const response = await fetch("http://coursescheduler.site/api/getPlan", {
